...
setInterval(async() => {
    ...
    let result = await postImgNumber('http://localhost:3002/moveChar2', inputdata);
    if(result < YcurPos) {
        Yprev();
    } else {
        Ynext();
    ...
    // 상대방 캐릭터 complete 정보
    result = await sendAjax('http://localhost:3002/makeChar2', inputdata);

    if(result === "true") {
        console.log('상대방 complete')
        complete2();
    }

    // 나와 상대방 <complete> 확인
    inputdata = {test:1};
    result = await sendAjax('http://localhost:3002/MYcomplete', inputdata);
